...
public class PipelineTests
{
    ...
    {
        Should.Throw<InvalidOperationException>(() => new UmlamuliServiceConfiguration().AddOpenBehavior(typeof(NotAnOpenBehavior)));
    }

    [Fact]
    public void Should_throw_when_adding_non_open_stream_behavior()
    {
        Should.Throw<InvalidOperationException>(() => new UmlamuliServiceConfiguration().AddOpenBehavior(typeof(NotAnOpenStreamBehavior)));
    }

    [Fact]
    public void Should_throw_when_adding_random_generic_type_as_open_behavior()
    {
        Should.Throw<InvalidOperationException>(() => new UmlamuliServiceConfiguration().AddOpenBehavior(typeof(List<string>)));
    }

    [Fact]
    public void Should_handle_open_behavior_registration()
    {
        var cfg = new UmlamuliServiceConfiguration();
    ...
    public async Task Should_register_correctly()
    {
        ...
        services.AddSingleton(new Umlamuli.Tests.PipelineTests.Logger());
        services.AddSingleton(new Umlamuli.Tests.StreamPipelineTests.Logger());
        services.AddSingleton(new Umlamuli.Tests.SendTests.Dependency());
