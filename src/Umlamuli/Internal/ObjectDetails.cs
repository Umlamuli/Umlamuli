//-----------------------------------------------------------------------
// <copyright file="ObjectDetails.cs" company="Umlamuli">
...
internal class ObjectDetails : IComparer<ObjectDetails>
{
    ...
    {
        if (x.AssemblyName == AssemblyName && y.AssemblyName != AssemblyName) return -1;

        if (x.AssemblyName != AssemblyName && y.AssemblyName == AssemblyName) return 1;
        if (x.AssemblyName != AssemblyName && y.AssemblyName != AssemblyName) return 0;
    ...
    {
        if (Location is null || x.Location is null || y.Location is null) return 0;

        if (x.Location.StartsWith(Location, StringComparison.Ordinal) &&
            !y.Location.StartsWith(Location, StringComparison.Ordinal)) return -1;

        if (!x.Location.StartsWith(Location, StringComparison.Ordinal) &&
            y.Location.StartsWith(Location, StringComparison.Ordinal)) return 1;
        if (x.Location.StartsWith(Location, StringComparison.Ordinal) &&
            y.Location.StartsWith(Location, StringComparison.Ordinal)) return 0;
    ...
    private int CompareByLocation(ObjectDetails x, ObjectDetails y)
    {
        ...
        if (Location.StartsWith(x.Location, StringComparison.Ordinal) &&
            !Location.StartsWith(y.Location, StringComparison.Ordinal)) return -1;
